...
async function main() {
  ...
  const results = await mapWithConcurrency(
    ...
    async (s) => {
      ...
      if (art) {
        resolved += 1;
        process.stdout.write(`  ✓ ${code.padEnd(6)} ${s.name}\n`);
      } else {
        skipped += 1;
        process.stdout.write(
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        _doc:
          "Per-set art_crop URLs used as backgrounds on the set-picker tiles. Generated by `node scripts/build-set-art.mjs`. Pages read this static map at SSR time — no live Scryfall calls. Re-run when new sets are added to refresh.",
